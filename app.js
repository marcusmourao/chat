...
nsp.on('connection', function(socket){
    ...
    nsp.emit('conect', username, user);    //envia a lista de usuário onlines para o cliente

    socket.on('disconnect', function(){
        nsp.emit('leave chat', user + " saiu da conversa"); // emite uma mensagem para os demais clientes avisando a saida do chat
        count --;                                          // atualiza o número de usuário conectados
        ...
        nsp.emit("update", username);                       // atualiza os clientes

    });

    socket.on('chat message', function(msg){    // emite uma mensagem para todos os clientes conectados
        nsp.emit('chat message', msg, user);     // informando a mensagem e o usuário que a enviou
    });
